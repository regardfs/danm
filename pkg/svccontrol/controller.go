...
//                      //
//////////////////////////
func (c *Controller) EpCheckUpdate(ipAddr string, ip6Addr, eps *corev1.Endpoints, pod *corev1.Pod, early bool) error {
  ...
  host := getPodHost(pod)
	...
	if PodReady(pod) || early {
		eps.Subsets[0].Addresses = append(eps.Subsets[0].Addresses, createChangedEpAddressList(ipAddr, ip6Addr, host, eps, targetRef))
	} else {
		eps.Subsets[0].NotReadyAddresses = append(eps.Subsets[0].NotReadyAddresses, createChangedEpAddressList(ipAddr, ip6Addr, host, eps, targetRef))
	}
...
func (c *Controller) MakeNewEps(svc *corev1.Service, des []*danmv1.DanmEp) (corev1.Endpoints) {
    ...
    if PodReady(pod) || svc.Annotations[TolerateUnreadyEps] == "true" {
      readyEpAddrs = append(readyEpAddrs, createChangedEpAddressList(strings.Split(de.Spec.Iface.Address, "/")[0], de.Spec.Iface.AddressIPv6, "/")[0], hostName, nil, targetRef))
    } else {
      notReadyEpAddrs = append(notReadyEpAddrs, createChangedEpAddressList(strings.Split(de.Spec.Iface.Address, "/")[0], de.Spec.Iface.AddressIPv6, "/")[0], hostName, nil, targetRef))
    }
	}
  ...
  ipAddr, ip6Addr := getIpsFromDanmEp(de)
	...
	for _, ep := range epsList {
      ...
      epNew.Subsets[0].Addresses = deleteFromEpAddressList(ipAddr, ip6Addr, index, address, epNew.Subsets[0].Addresses)
		...
		for i, a := range ep.Subsets[0].NotReadyAddresses {
	...
	if len(epList) > 0 {
    // TODO: this is kind of brute force. We should only retry if the returned error is the OptimisticLocErrorMsg
		err = c.UpdateEndpointsList(epList)
    if err != nil {
      glog.Errorf("Del danmep conflict !!!! %s %s %s+v", ipAddr, ip6Addr, epList)
...
func isIpInEp(ip string, eps *corev1.Endpoints) {
...
func createChangedEpAddressList(v4Address, v6Address, host string, pod *corev1.Pod, eps *corev1.Endpoints, targetRef *&corev1.ObjectReference) []corev1.EndpointAddress {
  var addresses []corev1.EndpointAddress
  if eps == nil || (v4Address != "" && !isIpInEp(v4Address, eps)) 
    addresses = append(addresses, corev1.EndpointAddress{IP: v4Address, Hostname: host, TargetRef: targetRef})
  }
  if eps == nil || (v6Address != "" && !isIpInEp(v6Address, eps)) {
    addresses = append(addresses, corev1.EndpointAddress{IP: v6Address, Hostname: host, TargetRef: targetRef})
  }
  return addresses
}
...
func deleteFromEpAddressList(v4Address, v6Address, string, index int, address string, epAddrs []corev1.EndpointAddress) []corev1.EndpointAddress {
  epAddrNew := epAddrNew.DeepCopy()
  if v4Address == address || v6Address == address {
    epAddrNew = append(epAddrNew[:index], epAddrNew[index+1:]...)
  }
  return epAddrNew
}
